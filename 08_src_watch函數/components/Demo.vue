  ...
  setup() {
    ...
    */
    ...
    /* watch(
      () => person.age,
      (newValue, oldValue) => {
        console.log("person中的age變化了", newValue, oldValue);
      }
    ); */

    // 情況五: 監視reactive所定義的一個響應式數據中的某些屬性
    /* watch([() => person.name, () => person.age], (newValue, oldValue) => {
      console.log("person中的age或name變化了", newValue, oldValue);
    }); */
    ...
    /* watch(
      () => person.job,
      (newValue, oldValue) => {
        console.log("person中的job變化了", newValue, oldValue);
      },
      { deep: true } //此處由於監視的是reactive所定義的對象中的某個屬性, 所以deep配置有效
    ); */
